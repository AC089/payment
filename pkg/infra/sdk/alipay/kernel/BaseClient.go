...
import (
	...
	"io"
	"io/ioutil"
	"payment/pkg/infra/sdk/alipay/tea"
	Sort "sort"
	"strings"
	"time"
)
...
func (client *Client) GetConfig(key string) string {
	if key == "protocol" {
		return client.Protocol
	} else if key == "gatewayHost" {
		return client.GatewayHost
	} else if key == "appId" {
		return client.AppId
	} else if key == "signType" {
		return client.SignType
	} else if key == "alipayPublicKey" {
		return client.AlipayPublicKey
	} else if key == "merchantPrivateKey" {
		return client.MerchantPrivateKey
	} else if key == "merchantCertPath" {
		return client.MerchantCertPath
	} else if key == "alipayCertPath" {
		return client.AlipayCertPath
	} else if key == "alipayRootCertPath" {
		return client.AlipayRootCertPath
	} else if key == "notifyUrl" {
		return client.NotifyUrl
	} else if key == "encryptKey" {
		return client.EncryptKey
	} else {
...
func (client *Client) GetMerchantCertSN() string {
	return client.MerchantCertSN
}

func (client *Client) GetAlipayCertSN(alipayCertSN map[string]string) string {
	return client.AlipayCertSN
}

func (client *Client) GetAlipayRootCertSN() string {
	return client.AlipayRootCertSN
}

func (client *Client) ExtractAlipayPublicKey(alipayPublicKey string) string {
	return client.AlipayCertSN
}

func (client *Client) IsCertMode() bool {
	if len(client.AlipayCertSN) != 0 {
		return true
	}
	return false
}
...
func AsyncVerify(sign string, content string, alipayPublicKey string) bool {
	signBytes, err := base64.StdEncoding.DecodeString(sign)
	if err != nil {
		return false
	}
