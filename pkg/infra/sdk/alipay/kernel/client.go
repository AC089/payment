...
import "payment/pkg/infra/sdk/alipay/tea"
...
func (client *Client) Execute(method string, textParams map[string]string, bizParams map[string]string) (_result *AlipayOpenApiGenericResponse, _err error) {
	...
	for _retryTimes := 0; tea.AllowRetry(_runtime["retry"], _retryTimes); _retryTimes++ {
		...
		_resp, _err = func()(*AlipayOpenApiGenericResponse, error){
			...
			systemParams := map[string]string{
				"method": method,
				"app_id": client.GetConfig("appId"),
				"timestamp": GetTimestamp(),
				"format": "json",
				"version": "1.0",
				"alipay_sdk": GetSdkVersion(),
				"charset": "UTF-8",
				"sign_type": client.GetConfig("signType"),
				"app_cert_sn": client.GetMerchantCertSN(),
				"alipay_root_cert_sn": client.GetAlipayRootCertSN(),
			}
			request_.Protocol = client.GetConfig("protocol")
			request_.Method = "POST"
			request_.Pathname = "/gateway.do"
			request_.Headers = map[string]string{
				"host": client.GetConfig("gatewayHost"),
				"content-type": "application/x-www-form-urlencoded;charset=utf-8",
			}
			request_.Query = SortMap(tea.Merge(map[string]string{
				"sign": Sign(systemParams, bizParams, textParams, client.GetConfig("merchantPrivateKey")),
			},systemParams,
				textParams))
			...
			if client.IsCertMode() {
				if Verify(respMap, client.ExtractAlipayPublicKey(client.GetAlipayCertSN(respMap))) {
				...
				if Verify(respMap, client.GetConfig("alipayPublicKey")) {
					_result = &AlipayOpenApiGenericResponse{}
					_body := ToRespModel(respMap)
					_err = tea.Convert(_body, &_result)
					return _result, _err
